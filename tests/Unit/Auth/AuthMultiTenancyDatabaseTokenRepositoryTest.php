...
use App\Packages\MultiTenancyPasswordReset\MultiTenancyDatabaseTokenRepository;
...
class AuthMultiTenancyDatabaseTokenRepositoryTest extends TestCase
{
    ...
    public function testCreateInsertsNewRecordIntoTable()
    {
        ...
        $query->shouldReceive('where')->with($this->getTenancyKey(), $this->getTenancyValue())->andReturn($query);
    ...
    {
        return new MultiTenancyDatabaseTokenRepository(
            m::mock(Connection::class),
            m::mock(Hasher::class),
            'table', 'key');
    }

    protected function getTenancyKey()
    {
        return 'tenancy_id';
    }

    protected function getTenancyValue()
    {
        return 1;
    }
    ...
    {
        $user = $user = m::mock(CanResetPassword::class);
        $user->{$this->getTenancyKey()} = $this->getTenancyValue();
